...
export class CakePackage extends PackageEntity implements Interactable {
  ...
  private buildVisual(): void {
    ...
    this.root.add(part(UNIT_CYLINDER, 'coral', new THREE.Vector3(0.58, 0.18, 0.58), 0.25));
    this.root.add(part(UNIT_CYLINDER, 'butter', new THREE.Vector3(0.48, 0.16, 0.48), 0.52));
    this.root.add(part(UNIT_TORUS, 'cream', new THREE.Vector3(0.5, 0.12, 0.5), 0.48));
    for (const x of [-0.28, 0, 0.28]) { const candle = part(UNIT_CYLINDER, 'mint', new THREE.Vector3(0.045, 0.18, 0.045), 0.88, x); this.root.add(candle); const flame = part(UNIT_CONE, 'emblemGold', new THREE.Vector3(0.08, 0.14, 0.08), 1.16, x); this.root.add(flame); }
