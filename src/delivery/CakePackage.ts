...
export class CakePackage extends PackageEntity implements Interactable {
  ...
  addExternalStress(multiplier:number):void{this.externalStress=Math.max(this.externalStress,multiplier);}
  ...
  update(dt: number, carried: boolean, anchor?: THREE.Object3D, stress = 0): void {
    ...
    if (carried && anchor) {
      ...
      this.wobbleVelocity += (stress * (this.externalStress||1) * 0.035 - this.wobble * 7) * dt;this.externalStress=0;
